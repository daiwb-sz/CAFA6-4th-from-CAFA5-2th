...
if __name__ == '__main__':
    ...
    print(f"当前本体GO术语数：{len(G.terms_list)}")
    ...
    print(f"找到的parquet文件数：{len(flist)}")
    print(f"根节点为1的样本数：{len(train_sl)}")
 
    # get priors
    # 50 150 800
    prior_cnd = joblib.load(os.path.join(helpers_path, f'real_targets/{G.namespace}/prior.pkl')) # 0.25
    nulls = joblib.load(os.path.join(helpers_path, f'real_targets/{G.namespace}/nulls.pkl'))
    prior_raw = prior_cnd * (1 - nulls) # 0.05
...
###################
    ...
    for oof_pred, idx, cnd in preds:
        print(f"基础模型预测形状：{oof_pred.shape}")
        print(f"topk GO术语数：{len(idx)}")
...
######################
    ...
    print(f"验证集蛋白ID数：{len(ids_to_take)}")
    print(f"验证集Dataset样本数：{len(val_ds)}")
