...
class Graph:
    ...
    def __init__(self, namespace, terms_dict, ia_dict=None, orphans=False):
        ...
        for id1, id2, ns in rel_list:
            if self.terms_dict.get(id2):
                ...
                if not self.dag[i, j]:  # 如果边不存在才添加
                    self.dag[i, j] = 1
                    self.terms_list[i]['adj'].append(j)
                    self.terms_list[j]['children'].append(i)
                    logging.debug("i,j {},{} {},{}".format(i, j, id1, id2))
                else:
                    logging.debug('Duplicate edge skipped: {}->{}'.format(id1, id2))
    ...
    def top_sort(self):
        ...
        if visited == rows:
            ...
            print(f"✅ 拓扑排序完成，顺序长度：{len(self.order)}")
            # # 验证2112和2116的顺序（调试）
            # if 2112 in self.order and 2116 in self.order:
            #     pos_2112 = self.order.index(2112)
            #     pos_2116 = self.order.index(2116)
            #     print(f"🔍 2112（父）位置：{pos_2112}，2116（子）位置：{pos_2116}，子是否在父后面：{pos_2116 > pos_2112}")
        else:
