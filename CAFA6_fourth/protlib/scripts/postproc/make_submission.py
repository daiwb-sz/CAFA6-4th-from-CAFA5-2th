...
import pandas as pd  # 已导入pandas，无需再导入cudf
import yaml
...
if __name__ == '__main__':
    ...
    # ========== 核心修改4：移除cudf.from_pandas，直接使用pandas拼接结果 ==========
    mapper = pd.concat(mapper)

    # goa leak
    # ========== 核心修改5：cudf.read_csv → pd.read_csv ==========
    goa = pd.read_csv(
    ...
    # gq51 dataset
    # ========== 核心修改6：cudf.read_csv → pd.read_csv ==========
    qg = pd.read_csv(
    ...
    # diff
    # ========== 核心修改7：cudf.read_csv → pd.read_csv ==========
    diff = pd.read_csv(
