    ...
    for n, i in tqdm.tqdm(enumerate(range(0, vec_train_protein_ids.shape[0], args.batch_size))):
        ...
        for ont in ontologies:
            ...
            if args.propagate:
                ...
                # 只看第一个批次的第一批蛋白
                print("传播后检查 GO:0006338 (2116) 和 GO:0006325 (2112)")
                print("样本中 2116==1 但 2112!=1 的数量：", np.sum((trg[:,2116]==1) & (trg[:,2112]!=1)))
            ...
            for k, node in enumerate(ont.terms_list):
                ...
                if len(adj) > 0:
                    ...
                    # # ========== 新增调试代码 ==========
                    # sum_trg_na_k = np.nansum(trg[na, k])
                    # if sum_trg_na_k != 0:
                    #     print(f"断言失败！术语k索引：{k}，术语ID：{ont.terms_list[k]['id']}")
                    #     print(f"na对应的蛋白质数量：{len(na)}")
                    #     print(f"trg[na, k]求和结果：{sum_trg_na_k}")
                    #     print(f"k的父节点列表（adj）：{adj}")
                    #     # 打印前5个矛盾的蛋白质值
                    #     if len(na) > 0:
                    #         print(f"trg[na[:5], k]的值：{trg[na[:5], k]}")
                    #     # 可选：直接退出，避免循环打印
                    #     sys.exit(1)
                    # # ========== 调试代码结束 ==========
