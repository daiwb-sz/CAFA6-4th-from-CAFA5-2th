...
def get_embeddings(model, tokenizer, seq, max_len=5120):
    # sequence_examples = [" ".join(list(re.sub(r"[UZOB]", "X", seq)))]

    seq = re.sub(r"[UZOB]", "X", seq)
    if len(seq) > max_len:
        # 策略1：保留N端（生物学上N端通常是功能关键区）
        seq = seq[:max_len]
        # 策略2（可选）：保留两端各500（兼顾N/C端）
        # seq = seq[:500] + seq[-500:]
    
    sequence_examples = [" ".join(list(seq))]
...
if __name__ == '__main__':
    ...
    tokenizer = T5Tokenizer.from_pretrained('/home/dwb/workspace/kaggle/PLMs/prot_t5_xl_half_uniref50-enc', do_lower_case=False)
    model = T5EncoderModel.from_pretrained("/home/dwb/workspace/kaggle/PLMs/prot_t5_xl_half_uniref50-enc").to(device)
    model.eval()
