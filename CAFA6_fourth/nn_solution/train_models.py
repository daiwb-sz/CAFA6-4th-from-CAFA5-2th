...
if __name__ == '__main__':
    ...
    n_samples_to_consider = 146937  # 1000 #   50_000#   142246 #    downsampling - might be useful for debug:  small number - fast run
    ...
    def model_predict_pytorch(model, XX, model_config, str_model_id='', verbose=1000):
        ...
        # 新增：维度校验
        assert Y_pred.shape[0] == XX.shape[0], f"预测样本数{Y_pred.shape[0]} != 输入样本数{XX.shape[0]}"
        assert Y_pred.shape[1] == Y.shape[1], f"预测标签数{Y_pred.shape[1]} != 目标标签数{Y.shape[1]}"
...
#################################
    if (mode_submit is not None) and (mode_submit != False):
        ...
        if X_submit is not None:
            n_test_samples = X_submit.shape[0]  # 动态获取测试集样本数
        else:
            # 兜底：从submit_protein_ids获取（如果X_submit未加载）
            n_test_samples = len(submit_protein_ids)
        
        print("请注意n_test_samples:", n_test_samples)
##################################

        Y_submit = np.zeros((n_test_samples, Y.shape[1]), dtype=np.float16)  # Predictions for submission will be stored here
